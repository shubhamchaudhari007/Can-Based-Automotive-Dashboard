 ...
 * Author: HP
 *
 * Created on 1 April, 2026, 1:07 PM
 */
...
void init_can(void) {
    ...
    RXB0CONbits.RXM0 = 1;    //when we disable the filter then the RXBOFUL bit is not automatically becomes 1 at receiving time
    RXB0CONbits.RXM1 = 1;      //using these two bits it will enable that behaviour of RXBOFUL register becomes 1 when data is receive
}
